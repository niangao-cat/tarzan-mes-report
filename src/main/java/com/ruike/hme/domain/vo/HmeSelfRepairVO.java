package com.ruike.hme.domain.vo;

import io.swagger.annotations.ApiModelProperty;
import lombok.Data;
import org.hzero.boot.platform.lov.annotation.LovValue;
import org.hzero.export.annotation.ExcelColumn;
import org.hzero.export.annotation.ExcelSheet;

import java.io.Serializable;

/**
 * 自制件返修统计报表
 *
 * @author xin.t@raycuslaser 2021/7/5 17:17
 */
@Data
@ExcelSheet(title = "自制件返修统计报表")
public class HmeSelfRepairVO implements Serializable {
    private static final long serialVersionUID = -6831676393411581600L;

    @ApiModelProperty(value = "返修SN")
    @ExcelColumn(zh = "返修SN",order = 1)
    private String repairSnNum;

    @ApiModelProperty(value = "EO标识")
    @ExcelColumn(zh = "EO标识",order = 2)
    private String identification;

    @ApiModelProperty(value = "型号")
    @ExcelColumn(zh = "型号",order = 3)
    private String materialModel;

    @ApiModelProperty(value = "产品编码")
    @ExcelColumn(zh = "产品编码",order = 4)
    private String materialCode;

    @ApiModelProperty(value = "产品描述")
    @ExcelColumn(zh = "产品描述",order = 5)
    private String materialName;

    @ApiModelProperty(value = "条码有效性")
    @LovValue(lovCode = "WMS.FLAG_YN", meaningField = "enableFlagMeaning")
    private String enableFlag;

    @ApiModelProperty(value = "有效性含义")
    @ExcelColumn(zh = "条码有效性",order = 6)
    private String enableFlagMeaning;

    @ApiModelProperty(value = "在制标识")
    @LovValue(lovCode = "WMS.FLAG_YN", meaningField = "mfFlagMeaning")
    private String mfFlag;

    @ApiModelProperty(value = "在制标识含义")
    @ExcelColumn(zh = "在制标识",order = 7)
    private String mfFlagMeaning;

    @ApiModelProperty(value = "条码状态")
    @LovValue(lovCode = "HME.WIP_MTLOT_STATUS", meaningField = "materialLotCodeStatusMeaning")
    private String materialLotCodeStatus;

    @ApiModelProperty(value = "条码状态含义")
    @ExcelColumn(zh = "条码状态",order = 8)
    private String materialLotCodeStatusMeaning;

    @ApiModelProperty(value = "工单号")
    @ExcelColumn(zh = "工单号",order = 9)
    private String workOrderNum;

    @ApiModelProperty(value = "工单状态")
    @LovValue(lovCode = "MT.WO_STATUS",meaningField = "wordOrderStatusMeaning")
    private String workOrderStatus;

    @ApiModelProperty(value = "工单状态含义")
    @ExcelColumn(zh = "工单状态",order = 10)
    private String wordOrderStatusMeaning;

    @ApiModelProperty(value = "是否创建SN")
    @LovValue(lovCode = "WMS.FLAG_YN", meaningField = "createSnFlagMeaning")
    private String createSnFlag;

    @ApiModelProperty(value = "是否创建SN含义")
    @ExcelColumn(zh = "是否创建SN",order = 11)
    private String createSnFlagMeaning;

    @ApiModelProperty(value = "工单创建时间")
    @ExcelColumn(zh = "工单创建时间",order = 12)
    private String actualStartDate;

    @ApiModelProperty(value = "工单结束时间")
    @ExcelColumn(zh = "工单结束时间",order = 13)
    private String actualEndDate;

    @ApiModelProperty(value = "当前工位编码")
    @ExcelColumn(zh = "当前工位编码",order = 14)
    private String workcellCode;

    @ApiModelProperty(value = "当前工位描述")
    @ExcelColumn(zh = "当前工位描述",order = 15)
    private String workcellName;

    @ApiModelProperty(value = "所在仓库编码")
    @ExcelColumn(zh = "所在仓库编码",order = 16)
    private String warehouseCode;

    @ApiModelProperty(value = "所在仓库名称")
    @ExcelColumn(zh = "所在仓库名称",order = 17)
    private String warehouseName;

    @ApiModelProperty(value = "所在仓库类型")
    private String warehouseType;

    @ApiModelProperty(value = "所在仓库类型含义")
    @ExcelColumn(zh = "所在仓库类型",order = 18)
    private String warehouseTypeMeaning;

    @ApiModelProperty(value = "所在货位编码")
    @ExcelColumn(zh = "所在货位编码",order = 19)
    private String locatorCode;

    @ApiModelProperty(value = "所在货位名称")
    @ExcelColumn(zh = "所在货位名称",order = 20)
    private String locatorName;

    @ApiModelProperty(value = "事业部")
    @ExcelColumn(zh = "事业部",order = 21)
    private String areaName;

    @ApiModelProperty(value = "申请人ID")
    private Long createdBy;

    @ApiModelProperty(value = "申请人工号")
    @ExcelColumn(zh = "申请人工号",order = 22)
    private String createdId;

    @ApiModelProperty(value = "申请人名称")
    @ExcelColumn(zh = "申请人名称",order = 23)
    private String createdName;
}
