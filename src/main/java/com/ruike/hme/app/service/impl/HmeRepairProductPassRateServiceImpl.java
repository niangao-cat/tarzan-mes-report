...
public class HmeRepairProductPassRateServiceImpl implements HmeRepairProductPassRateService {
    ...
    public void repairProductPassRateExport(Long tenantId, HmeProdLinePassRateDTO dto, HttpServletResponse response) throws IOException {
        log.info(">>>>>>>>>>>>>>>>>>>>>>开始导出返修产品直通率报表数据");
        HSSFWorkbook workbook = new HSSFWorkbook();
        HSSFSheet sheet = workbook.createSheet("返修产品直通率报表");
        String fileName = "返修产品直通率报表" + DateUtil.format(new Date(), "yyyyMMdd") + ".xls";
        ...
        try {
            ...
            headerCell1.setCellValue("返 修 产 品 直 通 率 报 表");
            ...
            throw new CommonException("返修产品直通率报表导出报错" + e.getMessage());
        } finally {
            //操作结束，关闭文件
            fOut.flush();
            fOut.close();
        }
        log.info(">>>>>>>>>>>>>>>>>>>>>>>>>>返修产品直通率报表导出完成");
    }
    ...
    public void repairProductPassRateAsyncExport(Long tenantId, HmeProdLinePassRateDTO dto, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String prefix = uuid + "@用户-" + currentUserName + "@时间-" + DateUtil.format(CommonUtils.currentTimeGet(), "yyyyMMddHHmmss")+ "@" + exportTaskVO.getTaskName();
        String suffix = ".xlsx";
    ...
    public void repairProductDayPassRateExport(Long tenantId, HmeProdLinePassRateDTO2 dto, HttpServletResponse response) throws IOException, ParseException {
        log.info(">>>>>>>>>>>>>>>>>>>>>>开始导出返修日直通率报表数据");
        HSSFWorkbook workbook = new HSSFWorkbook();
        HSSFSheet sheet = workbook.createSheet("返修日直通率报表");
        String fileName = "返修日直通率报表" + DateUtil.format(new Date(), "yyyyMMdd") + ".xls";
        OutputStream fOut = null;
        try {
            //创建excel文件对象
            fOut = response.getOutputStream();
            ...
            Map<String, CellStyle> styles = ExcellUtils.createStyles(workbook);
            headerCell1.setCellStyle(styles.get("title"));
            headerCell1.setCellValue("返 修 日 直 通 率 报 表");
            ...
            throw new CommonException("返修日直通率报表导出报错" + e.getMessage());
        } finally {
            //操作结束，关闭文件
            fOut.flush();
            fOut.close();
        }
        log.info(">>>>>>>>>>>>>>>>>>>>>>>>>>返修日直通率报表导出完成");
    }
