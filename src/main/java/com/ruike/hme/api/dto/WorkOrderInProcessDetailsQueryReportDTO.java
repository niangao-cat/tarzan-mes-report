...
public class WorkOrderInProcessDetailsQueryReportDTO implements Serializable {
    ...
    @ApiModelProperty(value = "返修SN集合")
    private List<String> reworkMaterialLotCodeList;

    @ApiModelProperty(value = "售后返修标识")
    private String afFlag;

    @ApiModelProperty(value = "工单号,以逗号分隔的字符串")
    private String workOrderNum;

    @ApiModelProperty(value = "工单号集合,后端自用", hidden = true)
    ...
    public  void initParam() {
        reworkMaterialLotCodeList = StringUtils.isNotBlank(reworkMaterialLotCode) ? Arrays.asList(StringUtils.split(reworkMaterialLotCode, ",")) : null;
