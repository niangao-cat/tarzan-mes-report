...
public class HmeCommonReportController {
    ...
    @ApiOperation(value = "非标产品报表导出")
    @GetMapping(value = "/non-standard-product-report-export", produces = "application/json;charset=UTF-8")
    @Permission(level = ResourceLevel.ORGANIZATION)
    @ExcelExport(HmeNonStandardReportVO2.class)
    public ResponseEntity<List<HmeNonStandardReportVO2>> nonStandardProductReportExport(@PathVariable("organizationId") Long tenantId,
                                                                                        HmeNonStandardReportVO dto,
                                                                                        ExportParam exportParam,
                                                                                        HttpServletResponse response) {
        return Results.success(hmeCommonReportRepository.nonStandardProductReportExport(tenantId, dto));
    }
