...
public class HmePlanRateReportController extends BaseController {
    ...
    @ApiOperation(value = "计划达成率报表导出")
    @Permission(level = ResourceLevel.ORGANIZATION)
    @GetMapping("/query-export")
    public ResponseEntity<?> planRateReportExport(@PathVariable("organizationId") Long tenantId,
                                                  HmePlanRateReportRequestDTO dto,
                                                  HttpServletResponse response) throws IOException {
        hmePlanRateReportService.planRateReportExport(tenantId, dto, response);
