 ...
 **/
@Component
public class HmeProdLinePassRateRepositoryImpl implements HmeProdLinePassRateRepository {
    ...
    public HmeProdLinePassRateVO5 prodLinePassRateQuery(Long tenantId, HmeProdLinePassRateDTO dto) {
        ...
        if (Objects.isNull(dto.getDateFrom()) && Objects.isNull(dto.getDateTo())) {
            HmeProdLinePassRateDTO hmeProdLinePassRateDTO = hmeProdLinePassRateMapper.dateSlotQuery(tenantId);
        ...
        //根据所有的工序集合查询出每个工序与其下的物料的对应关系
        List<HmeProdLinePassRateVO9> hmeProdLinePassRateVO9s = hmeProdLinePassRateMapper.materialQuery(tenantId, processIdList, dto);
        ...
        if(org.apache.commons.collections4.CollectionUtils.isNotEmpty(hmeProdLinePassRateVO9s)){
            ...
            for (List<HmeProdLinePassRateVO9> split : splitList) {
                eoBatchList.addAll(hmeProdLinePassRateMapper.getEoByProcessAndMaterial(tenantId, split, dto));
            }
        }
        //根据eo与工序分批查询不良记录
        ...
        if(org.apache.commons.collections4.CollectionUtils.isNotEmpty(eoBatchList)){
            Map<String, List<HmeProdLinePassRateVO11>> eoProcessMap = eoBatchList.stream().collect(Collectors.groupingBy(t -> {
                return t.getEoId() + "," + t.getProcessId();
            }));
            ...
            for (List<HmeProdLinePassRateVO11> split : splitList) {
                ncRecordBatchList.addAll(hmeProdLinePassRateMapper.ncRecordByEoQuery(tenantId, split));
            }
                    ...
                    if (productionNum == 0) {
                        ...
                        for (HmeProdLinePassRateVO11 eo : eoList) {
                            ...
                            List<HmeProdLinePassRateVO12> ncRecordList = ncRecordBatchList.stream().filter(item -> eo.getEoId().equals(item.getEoId())
                                    && process.getOrganizationId().equals(item.getProcessId())).collect(Collectors.toList());
                            if (CollectionUtils.isEmpty(ncRecordList)) {
                                //如果找不到父不良记录，则此Eo合格
                                passNum++;
                                identificationList.add(eo.getIdentification());
                            } else {
                                //将找到的父不良记录根据process_method为空或（!=2且!=4），且COMPONENT_REQUIRED=N筛选数据
                                List<HmeProdLinePassRateVO12> ncList = ncRecordList.stream().filter(item -> ((StringUtils.isEmpty(item.getProcessMethod())) || ((!"2".equals(item.getProcessMethod())) && (!"4".equals(item.getProcessMethod())))) && "N".equals(item.getComponentRequired())).collect(Collectors.toList());
    ...
    public HmeProdLinePassRateVO8 prodLineDayPassRateQuery(Long tenantId, HmeProdLinePassRateDTO2 dto) throws ParseException {
        ...
        for (List<String> split : splitList) {
            eoBatchList.addAll(hmeProdLinePassRateMapper.getEoByProcess(tenantId, split,
