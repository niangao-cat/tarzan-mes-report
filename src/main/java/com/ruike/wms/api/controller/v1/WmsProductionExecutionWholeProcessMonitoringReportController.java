...
public class WmsProductionExecutionWholeProcessMonitoringReportController {
    ...
    @ApiOperation(value = "生产执行全过程监控报表导出")
    @GetMapping(value = "/list-export", produces = "application/json;charset=UTF-8")
    @Permission(level = ResourceLevel.ORGANIZATION)
    @ExcelExport(WmsProductionExecutionWholeProcessMonitoringReportVO.class)
    public ResponseEntity<List<WmsProductionExecutionWholeProcessMonitoringReportVO>> listExport(@PathVariable("organizationId") Long tenantId,
                                                          ExportParam exportParam,
                                                          HttpServletResponse response,
                                                          WmsProductionExecutionWholeProcessMonitoringReportDTO condition) {
        return Results.success(productionExecutionWholeProcessMonitoringReportRepository.listExport(tenantId, condition));
    }
}
