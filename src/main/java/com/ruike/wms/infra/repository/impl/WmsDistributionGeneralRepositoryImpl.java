...
import com.ruike.wms.infra.mapper.WmsDistributionGeneralMapper;
...
public class WmsDistributionGeneralRepositoryImpl implements WmsDistributionGeneralRepository {
    ...
    public WmsDistributionGeneralRepositoryImpl(WmsDistributionGeneralMapper wmsDistributionGeneralMapper) {
    ...
    public Page<WmsDistributionGeneralVO> pageList(Long tenantId, WmsDistributionGeneralQueryDTO dto, PageRequest pageRequest) {
        ...
        if (page.size() > 0) {
            ...
            // 按单据计算是否备齐
            docIdSet.forEach(id -> {
                List<WmsDistributionListQueryVO1> lineList = wmsDistributionGeneralMapper.selectDistribution(tenantId, id);
                suiteMap.put(id, lineList.stream().allMatch(rec -> rec.getAcutalQty().compareTo(rec.getQuantity()) >= 0) ? Constant.ConstantValue.YES : Constant.ConstantValue.NO);
            });
    ...
    @Override
    @ProcessLovValue
    public List<WmsDistributionGeneralVO> export(Long tenantId, WmsDistributionGeneralQueryDTO dto) {
        List<WmsDistributionGeneralVO> resultList = wmsDistributionGeneralMapper.selectListByCondition(tenantId, dto);
        if (CollectionUtils.isNotEmpty(resultList)) {
            Set<String> docIdSet = resultList.stream().map(WmsDistributionGeneralVO::getInstructionDocId).collect(Collectors.toSet());
            Map<String, String> suiteMap = new HashMap<>(docIdSet.size() * 2);
            // 按单据计算是否备齐
            docIdSet.forEach(id -> {
                List<WmsDistributionListQueryVO1> lineList = wmsDistributionGeneralMapper.selectDistribution(tenantId, id);
            ...
            resultList.forEach(rec -> rec.setSuiteFlag(suiteMap.get(rec.getInstructionDocId())));
        }
        return resultList;
    }
}
