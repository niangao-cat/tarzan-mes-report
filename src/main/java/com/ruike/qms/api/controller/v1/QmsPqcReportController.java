...
public class QmsPqcReportController extends BaseController {
    ...
    @ApiOperation(value = "导出巡检头部数据")
    @GetMapping(value = "/excel-export", produces = "application/json;charset=UTF-8")
    @Permission(level = ResourceLevel.ORGANIZATION, permissionLogin = true)
    @ExcelExport(QmsPqcReportVO6.class)
    public ResponseEntity<List<QmsPqcReportVO6>> pqcReportHeadDataExport(@PathVariable("organizationId") Long tenantId,
                                                                                   QmsPqcReportDTO dto,
                                                                                   HttpServletResponse response,
                                                                                   ExportParam exportParam){
        return Results.success(qmsPqcReportService.pqcReportHeadDataExport(tenantId, dto));
    }
    ...
    @ApiOperation(value = "巡检报表-折线图")
    @Permission(level = ResourceLevel.ORGANIZATION)
    @GetMapping(value = {"/echart/mes-report"}, produces = "application/json;charset=UTF-8")
    public QmsPqcReportChartVO2 pqcReportEchartQuery (@PathVariable(value = "organizationId") Long tenantId,
                                                            QmsPqcReportDTO dto){
        return qmsPqcReportRepository.pqcReportEchartQuery(tenantId, dto);
    }
}
